...
def main():
    ...
    random.seed(42)
    num_to_sample = int(len(transcripts_to_sample) * 0.30)
    ...
    is_british = [117, 65, 84]  # Added British accent IDs based on your previous query
    ...
    for transcript_id in filtered_transcripts:
        ...
        if transcript_id in has_narrator:
            evaluation['has_narrator'] = True

        evaluation['accent'] = "American"
        if transcript_id in is_british:
